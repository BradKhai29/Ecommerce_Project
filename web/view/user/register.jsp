                ...
                <section class="container-fluid rounded-3 p-5">
                    <div class="container-md h-custom">
                        <div class="row">
                            ...
                            <c:if var="test" test="${phoneNumberError != null}">
                                <div class="alert alert-danger alert-dismissible fade show" role="alert">
                                    <strong>${phoneNumberError}</strong>
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                </div>
                            </c:if>
                            <c:if var="test" test="${existUsername != null}">
                                <div class="alert alert-danger alert-dismissible fade show" role="alert">
                                    <strong>${existUsername}</strong>
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                </div>
                            </c:if>
                            <c:if var="test" test="${confirmPasswordError != null}">
                                <div class="alert alert-danger alert-dismissible fade show" role="alert">
                            ...
                            <div class="col-md-8 col-lg-6 col-xl-4 offset-1">
                                <form action="register" accept-charset="UTF-8" method="post">
                                    <div class="divider d-flex align-items-center mb-4">
                                        <h4 class="text-center fw-bold mx-3 mb-0"><span class="badge bg-primary bg-opacity-75">ĐĂNG KÝ BƯỚC ${sessionScope.step2 == null ? 1 : 2}</span></h4>
                                    </div>
                                    <c:if var="test" test="${sessionScope.step2 == null}">
                                        <!-- full name input -->
                                        <div class="form-outline mb-3">
                                            <input type="text" name="fullname" id="form3Example3" class="form-control form-control-lg"
                                                   placeholder="Họ và tên quý khách" value="${registerUser.fullname}" required/>
                                        </div>
                                        <!-- phone number input -->
                                        <div class="form-outline mb-3">
                                            <input type="text" name="phoneNumber" class="form-control form-control-lg"
                                                   placeholder="Số điện thoại" required value="${registerUser.phoneNumber}" />
                                        </div>
                                        <!-- address input -->
                                        <div class="form-outline mb-3">
                                            <input type="text" name="userAddress" class="form-control form-control-lg"
                                                   placeholder="Địa chỉ giao hàng" value="${registerUser.userAddress}" />
                                        </div>
                                        <!-- email input -->
                                        <div class="form-outline mb-3">
                                            <input type="email" name="email" class="form-control form-control-lg"
                                                   placeholder="Email" value="${registerUser.email}" required/>
                                        </div>
                                    </c:if>
                                    <c:if var="test" test="${sessionScope.step2 != null}">
                                        <div class="divider d-flex align-items-center my-4">
                                            <h4 class="text-center fw-bold mx-3 mb-0"><a href="register?register=true" class="badge bg-danger text-decoration-none bg-opacity-75">QUAY LẠI BƯỚC 1</a></h4>
                                        </div>
                                        <!-- username input -->
                                        <div class="form-outline mb-3">
                                            <input type="text" name="username" class="form-control form-control-lg"
                                                   placeholder="Tên đăng nhập" value="${registerUser.username}" required/>
                                        </div>
                                        <!-- password -->
                                        <div class="form-outline mb-3">
                                    ...
                                    <div class="text-center text-lg-start mt-4 pt-2">
                                        <c:choose>
                                            <c:when test="${sessionScope.step2 == null}">
                                                <div class="d-grid ">
                                                    <button type="submit" name="register" value="step1" class="btn btn-primary btn-lg fw-bold"
                                                            style="padding-left: 2.5rem; padding-right: 2.5rem;">Tiếp tục</button>
                                                </div>
                                            </c:when>
                                            <c:otherwise>
                                                <div class="d-grid">
                                                    <button type="submit" name="register" value="step2" class="btn btn-primary btn-lg fw-bold"
